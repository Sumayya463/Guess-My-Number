...
if(input < random){
   div.textContent = "Input is too LOW!";
}
if(input > random){
   div.textContent = "Input is too HIGH!";
} if(input == random){
   div.textContent = "HURRAY!! , You guessed my number.";
}

if (input !== random) {
    if (input >= random - 5 && input < random) {
        div.textContent = "You're getting closer! Try a higher guess.";
    } else if (input <= random + 5 && input > random) {
        div.textContent = "You're getting closer! Try a lower guess.";
    } 
}
}
